...
class FragmentAddAdmin : Fragment(), DatePickerDialog.OnDateSetListener {
    ...
    override fun onCreateView(
        ...
        code.doOnTextChanged { text, _, _, _ ->
            if(code.text.toString().isNotEmpty()) {
                email.setText(text.toString().plus(selectedEmail), null)
                val arrayAdapterSecond = ArrayAdapter(requireContext(),
                    R.layout.exposed_dropdown_menu_item_layout,emailList)
                email.setAdapter(arrayAdapterSecond)
            }
            else{
                email.text = null
            }
        ...
        email.setOnClickListener {
            if(code.text.toString().isEmpty()){
                ...
                code.setError("Please enter Admin Code first",null)
                code.requestFocus()
            }
        }

        email.setOnItemClickListener { parent, _, position, _ ->
            if(code.text.toString().isNotEmpty()){
                val selectedItem = parent.getItemAtPosition(position)
                selectedEmail = selectedItem.toString()
                email.setText(code.text.toString().plus(selectedItem),null)
                val arrayAdapterThird = ArrayAdapter(requireContext(),
                    R.layout.exposed_dropdown_menu_item_layout,emailList)
                email.setAdapter(arrayAdapterThird)
            }
        }
    ...
    private fun addAdminWithAuth(){
        ...
        auth.createUserWithEmailAndPassword(mailTxt,passTxt).addOnCompleteListener(requireActivity()) { task ->
            if(task.isSuccessful){
                val adminCredentials: MutableMap<String, Any> = HashMap()
                adminCredentials["Code"] = codeTxt
                adminCredentials["Email"] = emailTxt
                adminCredentials["UID"] = auth.currentUser!!.uid
                adminCredentials["Password"] = encryptedPasswordTxt
                db.collection("AdminLoginInfo").document(emailTxt).set(adminCredentials).addOnCompleteListener { _ ->
                    ...
                    db.collection("AdminInfo").document(codeTxt).set(adminInfo).addOnSuccessListener { _ ->
                        viewModel.emailData.observe(viewLifecycleOwner,{
                            db.collection("AdminLoginInfo").document(it).get().addOnCompleteListener { task ->
                                if(task.isSuccessful){
                                    val currentUserPassword = task.result.data?.getValue("Password").toString()
                                    val decryptedPassword: String = EncryptDecryptPassword.decrypt(
                                        currentUserPassword
                                    ).toString()
                                    val currentUserEmail = task.result.data?.getValue("Email").toString()
                                    auth.signOut()
                                    auth.signInWithEmailAndPassword(currentUserEmail,decryptedPassword).addOnCompleteListener {
        ...
        docRef.get()
            .addOnSuccessListener { document ->
                if (document.data?.getValue("Email").toString() == mailTxt) {
                    Snackbar.make(parentFrameLayout,"Email already Registered",Snackbar.LENGTH_SHORT).setBackgroundTint(getColorFromAttr(
                        R.attr.colorPrimary
                    )).show()
                    progressBar.visibility = View.GONE
                }else {
                    db.collection("AdminLoginInfo")
                        ...
                        .addOnSuccessListener { documents ->
                            ...
                            for (document_it in documents) {
                                Snackbar.make(parentFrameLayout,"Code already exist",Snackbar.LENGTH_SHORT).setBackgroundTint(getColorFromAttr(
                                    R.attr.colorPrimary
                                )).show()
                                progressBar.visibility = View.GONE
                                temp = true
