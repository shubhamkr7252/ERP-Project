...
class FragmentAddTeacher : Fragment(), DatePickerDialog.OnDateSetListener {
    ...
    private lateinit var teacherEmail: AutoCompleteTextView
    ...
    override fun onCreateView(
        ...
        val arrayAdapterEmail = ArrayAdapter(requireContext(),
            R.layout.exposed_dropdown_menu_item_layout,emailList)
        teacherEmail.setAdapter(arrayAdapterEmail)

        selectedEmail = emailList[0]

        teacherCode.doOnTextChanged { text, _, _, _ ->
            if(teacherCode.text.toString().isNotEmpty()) {
                teacherEmail.setText(text.toString().plus(selectedEmail), null)
                val arrayAdapterSecond = ArrayAdapter(requireContext(),
                    R.layout.exposed_dropdown_menu_item_layout,emailList)
                teacherEmail.setAdapter(arrayAdapterSecond)
            }
            else{
        ...
        teacherEmail.setOnClickListener {
            if(teacherCode.text.toString().isEmpty()){
                teacherEmail.dismissDropDown()
                teacherCode.setError("Please enter Teacher Code first",null)
                teacherCode.requestFocus()
            }
        }

        teacherEmail.setOnItemClickListener { parent, _, position, _ ->
            if(teacherCode.text.toString().isNotEmpty()){
                val selectedItem = parent.getItemAtPosition(position)
                selectedEmail = selectedItem.toString()
                teacherEmail.setText(teacherCode.text.toString().plus(selectedItem),null)
                val arrayAdapterThird = ArrayAdapter(requireContext(),
                    R.layout.exposed_dropdown_menu_item_layout,emailList)
                teacherEmail.setAdapter(arrayAdapterThird)
            }
        }
    ...
    private fun addTeacher() {
        ...
        val activityCall = (activity as ActivityTeacherList)

        auth.createUserWithEmailAndPassword(emailTxt,passTxt).addOnCompleteListener {
            if (it.isSuccessful){
                val teacherCredentials: MutableMap<String, Any> = HashMap()
                teacherCredentials["Code"] = codeTxt
                teacherCredentials["Email"] = emailTxt
                teacherCredentials["Password"] = encryptedPasswordTxt
                teacherCredentials["UID"] = auth.currentUser?.uid.toString()

                db.collection("TeacherLoginInfo").document(emailTxt).set(teacherCredentials).addOnCompleteListener {
                                        ...
                                        .addOnCompleteListener { task ->
                                            if(task.isSuccessful){
                                                ...
                                                auth.signInWithEmailAndPassword(email,decryptedPassword).addOnCompleteListener { _ ->
                                                    Snackbar.make(parentParentFrameLayout,"Teacher Added",Snackbar.LENGTH_SHORT)
                                                        .setBackgroundTint(
                                                            getColorFromAttribute(requireActivity(), R.attr.colorPrimary)
                                                        )
                                                        .show()
                                                    activityCall.getTeacherData()
                                                    activity?.onBackPressed()
                                                }
                                            }
                                        }
