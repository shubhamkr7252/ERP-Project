                                ...
                                if(itAlt.isSuccessful){
                                    ...
                                    for(doc in itAlt.result!!){
                                        ...
                                        db.collection("TeacherInfo").document(doc.data.getValue("Code").toString()).update(updates).addOnCompleteListener {
                                            progressBar.visibility = View.GONE
                                            Snackbar.make(parentParentFrameLayout,"Course Removed",Snackbar.LENGTH_SHORT).setBackgroundTint(activityCall.getColorFromAttr(R.attr.colorPrimary))
                                                .show()
                                            activity?.onBackPressed()
                                        }
                                    }
                                    if(temp){
                                        progressBar.visibility = View.GONE
                                        Snackbar.make(parentFrameLayout,"Course not found",Snackbar.LENGTH_SHORT).setBackgroundTint(activityCall.getColorFromAttr(R.attr.colorPrimary))
                                            .show()
                                    }
                                }
                            }
