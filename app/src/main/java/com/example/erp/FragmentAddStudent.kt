...
class FragmentAddStudent : Fragment(), DatePickerDialog.OnDateSetListener {
    ...
    private lateinit var addStudentMail: EditText
                        ...
                        override fun onEvent(value: QuerySnapshot?, error: FirebaseFirestoreException?) {
                            ...
                            for(dc : DocumentChange in value?.documentChanges!!){
                                if(dc.type == DocumentChange.Type.ADDED){
                                    ...
                                    val addStudentSemesterSelectAdapter = ArrayAdapter(requireActivity(), R.layout.exposed_dropdown_menu_item_layout, tempArrayListSemesterInfo)
...
//            addStudentBatchSelect.setText(savedInstanceState.getString("batch"),null)
//            addStudentName.setText(savedInstanceState.getString("name"),null)
//            Toast.makeText(activity, addStudentName.text.toString(),Toast.LENGTH_SHORT).show()
//            if(addStudentBatchSelect.text.toString().isNotBlank())
//                db.collection("BatchInfo").whereEqualTo("Name",addStudentBatchSelect.text.toString())
//                    .addSnapshotListener(object: EventListener<QuerySnapshot> {
//                        @SuppressLint("NotifyDataSetChanged")
//                        override fun onEvent(value: QuerySnapshot?, error: FirebaseFirestoreException?) {
//                            if(error != null){
...
//                                if(dc.type == DocumentChange.Type.ADDED){
//                                    tempArrayListSemesterInfo.addAll(dc.document.data.getValue("Semester") as Collection<String>)
//                                    val addStudentSemesterSelectAdapter = ArrayAdapter(requireActivity(), R.layout.dropdown_item, tempArrayListSemesterInfo)
//                                    addStudentSemesterSelect.setAdapter(addStudentSemesterSelectAdapter)
//                                }
//                                break
//                            }
...
//        outState.putString("batch", addStudentBatchSelect.text.toString())
//        outState.putString("name", addStudentName.text.toString())
//    }
    ...
    private fun checkStudentExist(){
        ...
        val docRef = db.collection("StudentLoginInfo").document(srnTxt)
        docRef.get()
    ...
    private fun registerStudent(){
        ...
        studentCredentials["Password"] = passTxt

        db.collection("StudentLoginInfo").document(srnTxt).set(studentCredentials).addOnCompleteListener {
            ...
            db.collection("StudentInfo").document(srnTxt).set(studentInfo).addOnSuccessListener {
                ...
                Snackbar.make(parentFrameLayout,"Error occurred",Snackbar.LENGTH_SHORT).setBackgroundTint(activityCall.getColorFromAttr(R.attr.colorPrimary)).show()
                addStudentProgressBar.visibility = View.GONE
            }
        }.addOnFailureListener {
            Snackbar.make(parentFrameLayout,"Error occurred",Snackbar.LENGTH_SHORT).setBackgroundTint(activityCall.getColorFromAttr(R.attr.colorPrimary)).show()
